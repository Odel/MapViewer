...
public class Graphics2D /*extends CacheableNode*/ {
    ...
    public static final void drawHoirizontalLine(int i, int j, int k, int l) {
    ...
    //draws white & red lines/boundries/walls
    public final static void drawVerticalLine(int i, int j, int k, int l) {
    	//System.out.println(i+","+j+","+k+","+l);
        if (i < topX || i >= bottomX)
            return;
        if (j < topY) {
    ...
    public static final void drawRectangleOutline(int i, int j, int k, int l, int i1) {;
        drawHoirizontalLine(i, j, k, i1);
        drawHoirizontalLine(i, (j + l) - 1, k, i1);
        drawVerticalLine(i, j, l, i1);
        drawVerticalLine((i + k) - 1, j, l, i1);
    }
    ...
    public static final void drawCircle(int xpos, int ypos, int radius, int color, int opacity) {
        int j1 = 256 - opacity;
        ...
        for (int l2 = j2; l2 <= k2; l2++) {
            int i3 = l2 - ypos;
            int j3 = (int) Math.sqrt(radius * radius - i3 * i3);
            int k3 = xpos - j3;
            if (k3 < 0)
                k3 = 0;
            int l3 = xpos + j3;
