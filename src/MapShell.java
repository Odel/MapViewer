...
public class MapShell extends Applet
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7589195682718415957L;
	public final void stop() {
        if (addFocusListener >= 0)
            addFocusListener = 4000 / addKeyListener;
    }

    public final void drawRedLoadingBar(int barprogress, String s) {  //draws "Loading" and "Rendering Map" texts
        while (FI == null) {
            FI = J().getGraphics();
            try {
                J().repaint();
        ...
        if (exit) {
            ...
            FI.fillRect(0, 0, xsize, ysize);
            exit = false;
        }
        Color color = new Color(140, 17, 17);
        int j = ysize / 2 - 18;
        FI.setColor(color);
        FI.drawRect(xsize / 2 - 152, j, 304, 34);
        FI.fillRect(xsize / 2 - 150, j + 2, barprogress * 3, 30);
        FI.setColor(Color.black);
        FI.fillRect((xsize / 2 - 150) + barprogress * 3, j + 2, 300 - barprogress * 3, 30);
        FI.setFont(font);
        FI.setColor(Color.white);
        FI.drawString(s, (xsize - fontmetrics.stringWidth(s)) / 2, j + 22);
    }
    ...
    public final void I(int i, int j) {
        xsize = i;
        ysize = j;
        SI = new MapFrame(this, xsize, ysize);
        FI = J().getGraphics();
        JI = new ProducingGraphicsBuffer(xsize, ysize, J());
...
/*        int i = mouseevent.getX();
    ...
    public void updateGraphicsPositions() {
    }
    ...
    public final void run() {
        ...
        drawRedLoadingBar(0, IResourceLoader.I(730));
        loadAndRenderMap();
        int i = 0;
        int j = 256;
        int k = 1;
